--------------------------------------------------------------------------
Prompting Using Generative AI
--------------------------------------------------------------------------

# Have a number of tools open

https://chat.openai.com/ (using ChatGPT Plus)

https://gemini.google.com/ (paid account)

# https://zapier.com/blog/gemini-vs-chatgpt/

https://claude.ai/ (paid account)

https://www.meta.ai/ (no login needed)

https://copilot.microsoft.com/ (free account)

https://www.perplexity.ai/ (free account)

# Perplexity AI and ChatGPT are both AI tools, but they differ significantly in their primary function. Perplexity primarily functions as an AI search engine, focusing on real-time information retrieval and research, while ChatGPT is a conversational AI chatbot designed for natural language interaction and creative content generation. 


-----------------

# The free tier offers a lot more now

https://help.openai.com/en/articles/9275245-using-chatgpt-s-free-tier-faq

# If you're on a paid ChatGPT plan (Plus, Business), you can switch between models with the model selector.

# Users on the Free tier will be defaulted to GPT-5 with a limit on the number of messages they can send.

# ChatGPT plans here
https://chatgpt.com/pricing/

# Thinking modes in ChatGPT
https://help.openai.com/en/articles/11909943-gpt-5-in-chatgpt



# Can they browse the internet? Same prompt for all

"""
Can you browse popular newspapers from the US and tell me what today's headlines are?
"""

# - ChatGPT plus
# - Gemini
# - Claude
# - Meta AI
# - Copilot
# - Perplexity


# All of them support web search


-------------------------------------------------------------
# Now switch to the paid account ChatGPT Plus

# Show the model drop down

# Go to this page to see explanations for the models
https://help.openai.com/en/articles/7864572-what-is-the-chatgpt-model-selector


# Show temporary chat

# Click on the bottom-left and show the current plan

# Click on "Settings" and show the following options

Personalization
Data Controls


---------------------------------------------
# ChatGPT Plus

# Show some examples of prompts 

# Very open-ended


# Back to GPT 5 (Very open-ended prompt, will give you very open ended responses)
What kind of software tools and technologies should I learn today?

# Provide some context with your prompt, you will get more useful responses
I like working with data analysis and interpretation. What tools and technologies should I learn today?

---------------------------

# New GPT-5 session - with no context

# It has no idea and will ask a follow up question
Could you recommend some places to stay there?

# It will suggest places to stay in Vermont (remembers context)
I have heard Vermont is beautiful at this time of the year

# Follow up
How should I get there from SFO?

# Note that it remembers the previous context


---------------------------
# Compare models on politics (even neutral questions)

# Ask the same question across the different models

A few years ago when President Milei was elected in Argentina there was a lot of optimism. But recently it seems like things have gone off track. What are your thoughts on this?

# All of them try to give balanced views

----------------------------------------------------------------
# Gemini

# Uses a completely different model
# Bard (now Gemini) used to be powered by Google’s LaMDA and PaLM 2 language models, but now  Gemini uses Gemini Flash 

https://ai.google.dev/gemini-api/docs/models


# Google’s Gemini language models are “natively multimodal”, which means it’s trained a variety of inputs, not just text. Google has also fine-tuned the model with more multimodel information. They can also access the internet in real time.

"""
Can you give me a quick overview of what's happening in Davos right now?
"""

# Click on 3 dots and double check responses 

# Notice that it highlights the sources

# This is fresh news as of yesterday


----------------------------------------------------------------
# Anthropic's Claude

# Claude is a family of large language models developed by Anthropic.[1] The first model was released in March 2023.

The latest Claude models are Claude Sonnet 4.5 and Claude Haiku 4.5, both released in October 2025. Claude Sonnet 4.5 is the most advanced model, optimized for complex coding and agentic tasks, while Claude Haiku 4.5 is a faster and more cost-effective option that matches the coding performance of the previous Sonnet 4 model. 

# These do have real time information and can browse newspapers

"""
Can you give me a quick overview of how exactly AWS broke the internet yesterday?

"""

# It does not know

# Upload a PDF

"The Wise Owl and the Impatient Rabbit.pdf"


Who are the main characters in this story?

What does Rosie learn at the end of the story?


----------------------------------------------------------------
# Microsoft Copilot

# Uses multiple Open AI's models and other models to power its chat 

# Copilot utilizes the Microsoft Prometheus model. This model is based on OpenAI's GPT-5, GPT-4o, GPT-4 Turbo, and GPT-4o large language models. It also uses a component called the Orchestrator, which generates search queries to combine Bing search results with the OpenAI models. In addition, Copilot uses the DALL-E 3 model for image generation. 


I need to quickly brush up on my Excel skills for a project. How do I start?


Can you give me reference links?


----------------------------------------------------------------
# Meta AI

# Completely free to use

# Can generate images when logged in



----------------------------------------------------------------
# PerplexityAI

https://www.perplexity.ai/help-center/en/articles/10352961-what-are-spaces


Can you give me a quick overview of how exactly AWS broke the internet yesterday?


----------------------------------------------------------------
# Leaderboard to compare models

https://lmarena.ai/?leaderboard
























